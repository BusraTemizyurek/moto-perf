...
export class RideCard {
  ...
  constructor(session: Session) {
    ...
    const dataDurationDiv = this.createDataPair(
      "Time",
      formatDuration(session.duration),
    );
    const dataLeanAngleDiv = this.createDataPair(
      "Max Lean Angle",
      `${session.maxLeanAngle}${String.fromCharCode(176)}`,
    );
    ...
    if (session.distance !== 0) {
      const dataDistanceDiv = this.createDataPair(
        "Distance",
        `${session.distance} km`,
      );
    ...
    if (session.speed !== 0) {
      const dataSpeedDiv = this.createDataPair(
        "Speed",
        `${session.speed} km/h`,
      );
